...
if rg -n --hidden --glob '!.git/**' --glob "!${internal_dir}/**" --glob '!scripts/check-internal-boundaries.sh' --glob '!.github/workflows/boundaries.yml' "${internal_prefix}" . >/tmp/evidra_gitops_boundaries_refs.out 2>/dev/null; then
  echo "[boundaries] found forbidden references outside ${internal_dir}:"
  cat /tmp/evidra_gitops_boundaries_refs.out
  exit 1
fi

echo "[boundaries] checking README links to internal path"
if [ -f README.md ] && rg -n "\]\(${internal_prefix}|\]\(\.?/${internal_prefix}|${internal_prefix}" README.md >/tmp/evidra_gitops_boundaries_readme.out 2>/dev/null; then
  echo "[boundaries] README.md links or references internal path:"
  cat /tmp/evidra_gitops_boundaries_readme.out
  exit 1
fi

echo "[boundaries] checking docs links to internal path"
if [ -d docs ] && rg -n "\]\(${internal_prefix}|\]\(\.?/${internal_prefix}|${internal_prefix}" docs >/tmp/evidra_gitops_boundaries_docs.out 2>/dev/null; then
  echo "[boundaries] docs/ links or references internal path:"
  cat /tmp/evidra_gitops_boundaries_docs.out
  exit 1
fi
